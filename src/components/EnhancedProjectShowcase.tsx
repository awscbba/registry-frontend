...
export const EnhancedProjectShowcase: React.FC<EnhancedProjectShowcaseProps> = ({
  ...
  useEffect(() => {
    ...
    } else if (project.name.toLowerCase().includes('certification') || project.name.toLowerCase().includes('developer')) {
      // Fallback: Add sample formSchema for certification projects without one
      setEnhancedProject(prev => ({
        ...prev,
        formSchema: {
          version: '1.0',
...
*Perfect for developers looking to advance their AWS expertise!*`,
  ...
  }, [project.formSchema, project.name]);
